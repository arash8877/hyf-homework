...
const numberOfMovies1980 = movies.filter((movie) => {
  return movie.year >= 1980 && movie.year <= 1989;
}).length;

console.log(numberOfMovies1980);

const newMoviesArr = movies.map((object) => {
  let tagValue;
  if (object.rating < 4) {
    tagValue = "Bad";
  } else if (object.rating >= 4 && object.rating < 7) {
    tagValue = "Average";
  } else {
    tagValue = "Good";
  }
  return { ...object, tag: tagValue };
});
console.log(newMoviesArr);

const ratingOver6 = movies
  .filter((movie) => {
...
const moviesWithDuplicatedWord = movies.filter((movie) => {
  const titleArr = movie.title.split(" ");
  return titleArr.some((word, index) => titleArr.indexOf(word) !== index);
});
console.log(moviesWithDuplicatedWord);


const sumRating = movies.reduce((a, movie) => {
  return a + movie.rating;
}, 0);
const averageRating = Math.round((sumRating / movies.length) * 10) / 10;
...
// let countAverage = 0;
// let countBad = 0;
// const coRating = newMoviesArr.forEach((movie) => {
//   if (movie.tag === 'Good') {
//     countGood++
...
// })
// const ratings = {
//   goodMovies: countGood,
//   averageMovies: countAverage,
//   badMovies: countBad
// }
// console.log(ratings)
...
const ratingCount = newMoviesArr.reduce((a, movie) => {
   a[movie.tag]++
   return a
